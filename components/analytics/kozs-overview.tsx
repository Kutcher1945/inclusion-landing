...
export function KozsOverview({ kozs }: Props) {
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4">
      {KOZS_META.map(({ key, short, label, color }) => {
          ...
          <div key={key} className="bg-surface rounded-2xl border border-foreground/8 p-5 flex flex-col gap-3">
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <div className="text-xs text-foreground/40">{cat.total.toLocaleString("ru-RU")} объектов с оценкой</div>
              </div>
            </div>

            {/* Main metrics row */}
            <div className="flex items-stretch gap-2">
              <div className="flex-1 rounded-xl px-3 py-2" style={{ background: `color-mix(in srgb, ${color} 12%, transparent)` }}>
                <div className="text-xs text-foreground/50 mb-0.5">Доступен</div>
                <div className="text-xl font-bold tabular-nums" style={{ color }}>
                  {cat.accessible.toLocaleString("ru-RU")}
                </div>
              </div>
              <div className="flex-1 rounded-xl px-3 py-2 bg-foreground/[0.04]">
                <div className="text-xs text-foreground/50 mb-0.5">Частично</div>
                <div className="text-xl font-bold tabular-nums text-foreground/75">
                  {cat.partial.toLocaleString("ru-RU")}
                </div>
              </div>
              <div className="flex-1 rounded-xl px-3 py-2" style={{ background: "color-mix(in srgb, #ef4444 12%, transparent)" }}>
                <div className="text-xs text-foreground/50 mb-0.5">Не доступен</div>
            ...
            {(() => {
              ...
              return (
                <div className="w-full h-2.5 rounded-full bg-foreground/8 overflow-hidden flex">
                  ...
                  <div style={{ width: `${pctNone}%`, background: "color-mix(in srgb, var(--foreground) 15%, transparent)" }} className="h-full" title="Нет сведений" />
                </div>
              );
            })()}

            {/* Per passport mini breakdown */}
            <div className="pt-1 border-t border-foreground/5 space-y-1.5">
                ...
                ([type, d]) => {
                  ...
                  return (
                    <div key={type} className="flex items-center gap-2">
                      <span className="text-xs text-foreground/40 w-20 shrink-0">{PASSPORT_LABELS[type]}</span>
                      ...
                      <span className="text-xs tabular-nums text-foreground/50 w-8 text-right">
